...
gulp.task("pcscss", () => {
    $.fancyLog("-> Compiling PC scss");
    return gulp.src(pkg.paths.src.scss + pkg.vars.pcScssName)
        .pipe($.plumber({errorHandler: onError}))
        .pipe($.sourcemaps.init({loadMaps: true}))
        .pipe($.sass({
                includePaths: pkg.paths.scss
        ...
        .pipe($.sourcemaps.write("./"))
        .pipe($.size({gzip: true, showFiles: true}))
        .pipe(gulp.dest(pkg.paths.build.pc));
});
// css task - combine & minimize any distribution CSS into the public css folder, and add our banner to it
gulp.task("pccss", ["pcscss",], () => {
    $.fancyLog("-> Building PC css");
    return gulp.src(pkg.globs.distPcCss)
        .pipe($.plumber({errorHandler: onError}))
        .pipe($.newer({dest: pkg.paths.dist.pc + pkg.vars.sitePcCssName}))
        .pipe($.print())
        .pipe($.sourcemaps.init({loadMaps: true}))
        .pipe($.concat(pkg.vars.sitePcCssName))
        .pipe($.cssnano({
            discardComments: {
                removeAll: true
            },
        ...
        .pipe($.sourcemaps.write("./"))
        .pipe($.size({gzip: true, showFiles: true}))
        .pipe(gulp.dest(pkg.paths.dist.pc))
        .pipe($.filter("**/*.css"))
        .pipe($.livereload());
});
...
// uikit js task - minimize the uikit uri(s) into 'uikit' in the templates path
gulp.task("js-uikit", () => {
    $.fancyLog("-> Building UIkit js");
    return gulp.src(pkg.globs.uikit)
    .pipe($.plumber({errorHandler: onError}))
    .pipe($.jsmin())
    .pipe($.newer({dest: pkg.paths.dist.js + pkg.vars.siteUiKitName}))
    .pipe($.concat(pkg.vars.siteUiKitName))
    //.pipe($.header(banner, {pkg: pkg}))
    .pipe($.size({gzip: true, showFiles: true}))
...
gulp.task("js", ["js-inline", "js-babel", "js-uikit"], () => {
...
// Default task
gulp.task("default", ["css", "js", "pccss"], () => {
    $.livereload.listen();
