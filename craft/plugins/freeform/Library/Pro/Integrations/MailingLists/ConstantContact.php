...
namespace Solspace\Freeform\Library\Pro\Integrations\MailingLists;
...
class ConstantContact extends MailingListOAuthConnector
{
    ...
    protected function fetchLists()
    {
        ...
        $status = $response->getStatusCode();
        if ($status !== 200) {
            $this->getLogger()->log(LoggerInterface::LEVEL_ERROR, 'Could not fetch MailChimp lists', self::LOG_CATEGORY);
